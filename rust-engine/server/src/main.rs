...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let physics_engine = physics::PhysicsSolver::new(16384, 16384, gpu_vram_gb, headless, "@group(0) @binding(0) var<storage, read_write> data: array<f32>; @compute @workgroup_size(1) fn main() { data[0] = 0.0; }").await;
